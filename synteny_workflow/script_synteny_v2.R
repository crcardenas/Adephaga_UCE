...
#pdf(file=c(paste0(species1,"_",species2,"_syntenyplot.pdf")))
   ...
   #xaxis <- sp1_fai$V1
   #yaxis <- 
   #axis(side = 1, at = sp1_fai$V1, las= 2, labels=T)

  # Label the axes
   # axis(1, sp1_fai$V1, line = -0.2, tick = F)
   # axis(2, sp2_fai$V1, las = 2, tick = F)
   
   axis(1,sp1_fai$add[!grepl(sp1_fai$V1,pattern="unloc")]+
          sp1_fai$V2[!grepl(sp1_fai$V1,pattern="unloc")]/2,
        sp1_fai$V1[!grepl(sp1_fai$V1,pattern="unloc")],line = -0,tick = T)
   axis(2,sp2_fai$add+sp2_fai$V2/2,sp2_fai$V1,las=2,tick = T)
